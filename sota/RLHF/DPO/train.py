...
# ==========================================
# DPO Core Logic (Unchanged)
# ==========================================

def get_batch_logps(logits, labels, average_log_prob=False):
    """Calculate Log Probabilities of sequences"""
    # Logits and labels should be strictly aligned at this point, but fine-tune to prevent off-by-1 from shifting
...
class DPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        concatenated_batch = {}
        # Direct concatenation since Dataset ensures all Tensors are max_length
        ...
        all_logps = get_batch_logps(all_logits, concatenated_batch["labels"])
        ...
        with torch.no_grad():
            ...
            ref_logps = get_batch_logps(ref_logits, concatenated_batch["labels"])
            reference_chosen_logps = ref_logps[:len_chosen]
            reference_rejected_logps = ref_logps[len_chosen:]

        loss = dpo_loss(policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps, beta=self.beta)
...
if __name__ == '__main__':
    ...
    if os.path.exists(data_path) and os.path.exists(prompt_path):
        ...
        for json_data in json_datas:
            # === [Modification Start] Build Prompt with Chat Template ===
            messages = [
            ...
            # Result example: "<|im_start|>system\n...<|im_end|>\n<|im_start|>user\n...<|im_end|>\n<|im_start|>assistant\n"
        ...
        # Dummy data must also conform to format
    ...
    lora_config = LoraConfig(
        ...
        r=64,  # <-- Increase r value (e.g., 32, 64, 128)
        lora_alpha=64, # Recommended to set lora_alpha equal to or larger than r
        lora_dropout=0.1,
    ...
    # The 'prompt' passed in is already a complete string containing tags like <|im_start|>
    dataset = PreferenceDataset(datas, tokenizer, max_length=1024)

    output_dir = "sota/RLHF/models/DPO_output/"
